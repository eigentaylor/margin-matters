...
MAINTENANCE_PASSWORD = "ericadams2028"  # Change this to your desired password
...
EXPLANATION_TEXT = (
    ...
    "<br /><br />"
    "<b>Flip scenarios</b> are computed by solving a knapsack problem to find the minimum number of popular votes needed to flip enough states to change the electoral college outcome (either to produce a new winner or to simply break the majority the original winner had). We can choose the solution based on the minimum number of votes needed to flip or the minimum total percent of state margins flipped. For example: in 2024, if Harris had won AZ, GA, and NC, then it would have been an Electoral College tie. In each of those states, we'd only be flipping 1-3% of the votes. But that would have actually required more votes total (242,768) than flipping NV, MI, PA, and AK (only 145,408, which is nearly 100 thousand fewer, but the 22,217 votes in AK would have been 6.57% of all votes cast in that state), so we show both solutions under different metrics."
)
