    ...
    if getattr(params, "INTERACTIVE_TESTER", False):
        ...
        tester_ui = (
            "\n".join([
                ...
                '        <span id="pvVal" style="width:120px;text-align:left">EVEN</span>',
                '      </div>',
                ...
                f'    <div id="testerExplain" class="legend" style="font-size:0.95rem;text-align:left;color:var(--muted)">We only capped the maximum PV shift to ±{cap_pct}%.</div>',
                '  </div>',
                '</div>',
            ])
        )
