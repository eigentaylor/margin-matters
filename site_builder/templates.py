...
(function() {
  ...
  const sliderRange = document.getElementById('slider-range');
    ...
    // Create custom thumbs for the year range so the full-width native range
    // inputs do not steal pointer events. Custom thumbs forward value updates
    // to the underlying inputs (which remain the source of truth).
    (function(){
      try {
        const yearStartEl = document.getElementById('year-start');
        const yearEndEl = document.getElementById('year-end');
        const container = yearStartEl && yearStartEl.closest && yearStartEl.closest('.dual-range-slider');
        if (!yearStartEl || !yearEndEl || !container) return;

        // Make native inputs non-interactive (visual only). We still programmatically
        // update their values and dispatch events so existing handlers run.
        yearStartEl.style.pointerEvents = 'none';
        yearEndEl.style.pointerEvents = 'none';
        ...
        const minY = () => parseInt(yearStartEl.min, 10);
        const maxY = () => parseInt(yearStartEl.max, 10);

        const updateThumbs = () => {
          const mi = minY(); const ma = maxY();
          const s = Math.max(mi, Math.min(ma, parseInt(yearStartEl.value, 10) || mi));
          const e = Math.max(mi, Math.min(ma, parseInt(yearEndEl.value, 10) || ma));
        ...
        yearStartEl.addEventListener('input', updateThumbs);
        yearEndEl.addEventListener('input', updateThumbs);
        updateThumbs();
        ...
        attachThumb(thumbStart, yearStartEl);
        attachThumb(thumbEnd, yearEndEl);

        // Clicking on the track moves the nearest thumb (ignores clicks inside current range)
        container.addEventListener('pointerup', (ev) => {
          ...
          const s = parseInt(yearStartEl.value, 10);
          const e = parseInt(yearEndEl.value, 10);
          ...
          const targetInput = distStart <= distEnd ? yearStartEl : yearEndEl;
