(function(){
	...
	function runProbabilisticMode(){
		...
		// Build key based on current contestables + fixed picks
		const contestables = [];
		const seen = new Set();
		// Include state-level entries only for states without multiple districts (units length <=1)
		stateStore.forEach(st => {
			if (st.units && st.units.length > 1) return; // skip aggregated state if it has district breakdown
			if (st.status==='tossup' || st.strength!=='safe') {
				if (!seen.has(st.state)) {
					contestables.push({ id: st.state, ev: st.ev, margin: st.margin, status: st.status, strength: st.strength, baseStatus: st.baseStatus });
					seen.add(st.state);
				}
			}
		});
		// Include all unit-level entries (districts + any AL entries). Skip unit if duplicate of state already added.
		unitStore.forEach(u => {
			const parent = stateStore.get(u.state);
			// If this unit is exactly the state code and we already added state entry, skip to avoid duplication
			if (u.unit === u.state && seen.has(u.unit)) return;
			if (u.status==='tossup' || u.strength!=='safe') {
				if (!seen.has(u.unit)) {
					contestables.push({ id: u.unit, ev: u.ev, margin: u.margin, status: u.status, strength: u.strength, baseStatus: u.baseStatus });
					seen.add(u.unit);
				}
			}
		});
		contestables.sort((a,b)=>a.id.localeCompare(b.id));
		const fixed = []; // manual picks that are currently Dem/Rep lean or safe (decided by user outside base?)
		contestables.forEach(c => {
			if (c.status==='dem' && c.strength!=='tossup') fixed.push({ id:c.id, party:'dem'});
			else if (c.status==='rep' && c.strength!=='tossup') fixed.push({ id:c.id, party:'rep'});
		});
		const key = JSON.stringify({ thresh: currentThreshold, contestables: contestables.map(c=>[c.id,c.status,c.strength]), fixed });
		if (key === probCacheKey && lastProbResult){
			applyProbResults(lastProbResult);
			return;
		}
		probCacheKey = key;
		...
		let demWins=0, repWins=0, ties=0;
		const demCountsPer = new Map(); // id -> count assigned Dem in accepted sims
		const repCountsPer = new Map();
		// Pre-calc decided base EV outside contestables (safe states not in contestables set)
		let baseDemEV=0, baseRepEV=0;
		stateStore.forEach(st => {
			const included = (st.status==='tossup' || st.strength!=='safe');
			if (!included){
				if (st.status==='dem') baseDemEV += st.ev; else if (st.status==='rep') baseRepEV += st.ev;
			}
		});
		unitStore.forEach(u => {
			const included = (u.status==='tossup' || u.strength!=='safe');
			if (!included){ if (u.status==='dem') baseDemEV += u.ev; else if (u.status==='rep') baseRepEV += u.ev; }
		});
		// Extract arrays for speed
		const ids = contestables.map(c=>c.id);
		const evs = contestables.map(c=>c.ev);
		const margins2024 = contestables.map(c=>c.margin); // relative margin baseline
		const fixedMap = new Map(fixed.map(f=>[f.id, f.party]));
		const maxSims = PROB_MAX_SIMS;
		while (tried < maxSims && accepted < PROB_TARGET_ACCEPT){
			tried++;
			// Sample PV national shift
			const pv = randNormal(0, PV_STD);
			let demAdd=0, repAdd=0;
			let valid = true;
			const demAssigned = []; // bool
			for (let i=0;i<ids.length;i++){
				// sample state-specific noise
				const noise = randNormal(0, STATE_STD);
				const adjMargin = margins2024[i] + pv + noise; // >0 means Dem win
				let demWinsState = adjMargin > 0;
				const forced = fixedMap.get(ids[i]);
				if (forced){
					if (forced==='dem' && !demWinsState){ valid=false; break; }
					if (forced==='rep' && demWinsState){ valid=false; break; }
					demWinsState = forced==='dem';
				}
				demAssigned[i] = demWinsState;
				if (demWinsState) demAdd += evs[i]; else repAdd += evs[i];
			}
			if (!valid) continue; // reject simulation
			accepted++;
			const demTotal = baseDemEV + demAdd;
			const repTotal = baseRepEV + repAdd;
			if (demTotal>=270 && repTotal<270) demWins++; else if (repTotal>=270 && demTotal<270) repWins++; else if (demTotal===269 && repTotal===269) ties++; else {
				if (demTotal>repTotal) demWins++; else if (repTotal>demTotal) repWins++; else ties++;
			}
			for (let i=0;i<ids.length;i++){
				if (demAssigned[i]) demCountsPer.set(ids[i], (demCountsPer.get(ids[i])||0)+1);
				else repCountsPer.set(ids[i], (repCountsPer.get(ids[i])||0)+1);
			}
		}
		const runtime = performance.now()-t0;
		const result = {
			accepted, tried, demWins, repWins, ties, baseDemEV, baseRepEV,
			perState: ids.map(id => {
				const d = demCountsPer.get(id)||0; const r = repCountsPer.get(id)||0; const total = d+r; return { id, pDem: total? d/total: 0.5, sims: total };
			}), runtime
		};
		lastProbResult = result;
		applyProbResults(result);
	}
