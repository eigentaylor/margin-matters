(function () {
	...
	function formatMargin(x) { if (!isFinite(x)) return 'N/A'; if (Math.abs(x) < 1e-4) return 'EVEN'; const pct = (Math.abs(x) * 100).toFixed(1); return (x > 0 ? 'D+' : 'R+') + pct; }
	...
	function computePathCounts(demEV, repEV, tossups) {
		...
		tossups.forEach(st => { const ev = st.ev || 0; const next = new Map(); dp.forEach((count, sum) => { next.set(sum, (next.get(sum) || 0) + count); const newSum = sum + ev; next.set(newSum, (next.get(newSum) || 0) + count); }); dp = next; });
