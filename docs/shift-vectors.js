...
(function () {
  ...
  function arrowGlyph(v) { return v > 0 ? '▲' : (v < 0 ? '▼' : '–'); }
    tooltipEl.innerHTML = `
      ...
      <div style="word-break: break-word; white-space: pre-line;">Δ D votes: ${formatSigned(entry.dDelta)}</div>
      <div style="word-break: break-word; white-space: pre-line;">Δ R votes: ${formatSigned(entry.rDelta)}</div>
  <div style="word-break: break-word; white-space: pre-line;">Partisan shift (ΔD - ΔR): ${arrowGlyph(partisan)} ${formatSigned(partisan)} <span style="color:rgba(255,255,255,0.7)">(${partisanPct.toFixed(2)}%)</span></div>
      <div style="word-break: break-word; white-space: pre-line;">Turnout Δ (D+R): ${formatSigned(turnout)}</div>
      <div style="word-break: break-word; white-space: pre-line;">Vector length: ${formatPlain(entry.length)} votes</div>
      ${entry.totalDelta != null ? `<div style="word-break: break-word; white-space: pre-line;">Total Δ votes: ${formatSigned(entry.totalDelta)}</div>` : ''}
